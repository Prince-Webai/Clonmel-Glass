...
const InvoiceCalendar = () => {
  ...
  const invoicesByDate = useMemo(() => {
    ...
    invoices.forEach(inv => {
      try {
        if (inv.documentType === 'quote') return; // Don't show quotes on calendar
  ...
  try {
                      ...
                      {invs.length > 0 && (
                        <div className="mt-2">
                          <div className="text-[10px] font-black text-slate-900 truncate">
                            {(invs[0].customerName || 'Unknown').split(' ')[0]}
                          </div>
                          {invs.length > 1 && (
                      ...
                      >
                        <div className="flex justify-between items-start mb-4">
                          <div className="max-w-[140px]">
                            ...
                            <span className="block text-sm font-black text-white group-hover:text-brand-400 transition-colors truncate">{inv.customerName || 'Unknown'}</span>
                          </div>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-white/5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-black text-white">{formatCurrency(inv.balanceDue || 0)}</span>
                          </div>
                          <button
              ...
              <div className="mt-8 pt-8 border-t border-white/10 relative z-10">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                    ...
                    <span className="text-base font-black text-white">
                      {formatCurrency(selectedInvoices.reduce((sum, i) => sum + (i.balanceDue || 0), 0))}
                    </span>
                  </div>
                ...
                <div className="mt-6 flex items-center gap-3 p-4 bg-rose-500/10 rounded-2xl border border-rose-500/20">
                  ...
                  <p className="text-[9px] font-bold text-rose-200 leading-relaxed">
                    Priority notification: {selectedInvoices.filter(i => new Date(i.dueDate) < new Date() && i.status !== PaymentStatus.PAID).length} invoices require urgent follow-up today to maintain cash flow.
                  </p>
                </div>
              </div>
            </div>
    ...
    return (
      <div className="p-12 text-center">
        ...
        <h3 className="text-xl font-black text-slate-800 mb-2">Calendar Error</h3>
        <p className="text-slate-500 mb-6">Something went wrong loading the calendar view.</p>
        <pre className="bg-slate-100 p-4 rounded-xl text-left text-xs font-mono text-slate-600 overflow-auto max-w-2xl mx-auto">
